...
public class GameScreen extends Screen3d {
	...
	public void preDrawHook(float delta) {
		...
		/*if(camTarget.body == null){
			...
			System.err.println("WTF CAMTARGET BODY NULL, finalement �a l'air d'�tre juste pcq le camTarget en question a d�connect�");
		}*/
	}
	...
	public void postDrawHook(float delta) {
		...
		if(Gdx.input.isKeyPressed(Keys.TAB)){
			// affiche l'�cran de score par dessus le reste
	...
	public void resetCamTarget(){
		if(TestGame.get().getClientState() == ClientState.PLAYING) {
       		...
       		TestGame.get().getClientList().stream()
       				.filter(c -> c.state == ClientState.PLAYING) // SI T'ES PLAYING, �A IMPLIQUE QUE T'AS UN BPLAYER ET QUE T'ES PAS MORT
